...
const FoodList = () => {
  ...
  const EditFood = async () => {
    setLoading(true);
    try {
      const res = await axios.put(`http://localhost:5000/foods/${editFood._id}`, {
        id: foodId,
        name: foodName,
        price: foodPrice,
      });
  ...
  const DeleteFood = async () => {
    setLoading(true);
    try {
      const res = await axios.delete(`http://localhost:5000/foods/${deletedFood._id}`);
        ...
        <div className="flex flex-row mb-1 sm:mb-0 justify-between w-full">
          ...
          <div className="text-end">
            <div className="flex flex-col md:flex-row w-3/4 md:w-full max-w-sm md:space-x-3 space-y-3 md:space-y-0 justify-center">
              ...
              <button className="flex-shrink-0 px-4 py-2 text-base font-semibold text-white bg-green-600 shadow-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:ring-offset-green-200" type="button">
                Filter
              </button>
            </div>
          </div>
